...
腳本會自動取得題名與 README 路徑，新題目以執行當天作為首次完成日期，並依題號插入表格。若該題已存在，會保留首次完成日期，將最近複習日期更新為當天，並更新熟悉度與語言。省略個人筆記時保留原筆記，傳入空字串則清空筆記。

每次執行都會在個人頁面的「題目紀錄」之前重建「練習統計」，包含不重複的總練習題數及熟悉度 1–5 各自的題數。統計依目前的題目紀錄計算，不代表累積練習次數；請勿手動編輯 `progress-summary` 標記內的內容。手動修改題目紀錄後，統計會在下次執行此指令時更新。
